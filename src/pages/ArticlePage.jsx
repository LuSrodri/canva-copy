          ...
          <section className="mt-16">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {otherArticles.map(otherArticle => {
                const otherColors = colorClasses[otherArticle.color] || colorClasses.blue
                return (
                  <Link 
                    key={otherArticle.slug} 
                    to={`/blog/${otherArticle.slug}`}
                    className="group"
                  >
                    <Card className="h-full hover:shadow-lg transition-shadow">
                      <CardContent className="p-6 space-y-3">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${otherColors.bg} ${otherColors.text}`}>
